var h = Object.defineProperty;
var b = (t, A, o) => A in t ? h(t, A, { enumerable: !0, configurable: !0, writable: !0, value: o }) : t[A] = o;
var r = (t, A, o) => b(t, typeof A != "symbol" ? A + "" : A, o);
import { defineComponent as p, getCurrentInstance as D, onBeforeMount as f, onMounted as O, onBeforeUpdate as P, onUpdated as G, onBeforeUnmount as L, onUnmounted as N, version as T, useSlots as q, openBlock as j, createElementBlock as x, renderSlot as W } from "vue";
const R = "data:image/jpeg;base64,", H = { loadingImg: R, errorImg: "data:image/jpeg;base64,", delay: 500 }, k = { root: null, rootMargin: "0px", threshold: 0 };
var a = ((t) => (t.LOADING = "loading", t.LOADED = "loaded", t.ERROR = "error", t))(a || {}), l = ((t) => (t.IMG_ID = "data-img-id", t.ORIG_SRC = "data-orig-src", t.TIMEOUT_ID = "data-timeout-id", t.LOAD_STATUS = "data-load-status", t))(l || {});
function B(t, A = ["object", "array", "function", "date", "regexp", "error", "set", "map"]) {
  if (t === null || typeof t != "object" && typeof t != "function") return t;
  if (o = t, toString.call(o) === "[object Object]" && A.includes("object")) return Object.keys(t).reduce((e, g, i) => (e[g] = B(t[g], A), e), {});
  if (function(e) {
    return toString.call(e) === "[object Array]" || Array.isArray(e);
  }(t) && A.includes("array")) return t.reduce((e, g, i) => (e[i] = B(g, A), e), []);
  if (function(e) {
    return toString.call(e) === "[object Function]" || typeof e == "function";
  }(t) && A.includes("function"))
    return new Function("return " + t.toString())();
  if (function(e) {
    return toString.call(e) === "[object Date]" || e instanceof Date;
  }(t) && A.includes("date")) return new Date(t.getTime());
  if (function(e) {
    return toString.call(e) === "[object RegExp]" || e instanceof RegExp;
  }(t) && A.includes("regexp")) return new RegExp(t.source, t.flags);
  if (function(e) {
    return toString.call(e) === "[object Error]" || e instanceof Error;
  }(t) && A.includes("error")) {
    const e = { ...t };
    Object.getOwnPropertyNames(t).forEach((i) => {
      i === "stack" && (e[i] = t[i].split(`
`)), t.hasOwnProperty("k") && i !== "message" && i !== "name" && i !== "stack" && i !== "constructor" && (e[i] = t[i]);
    });
    const g = new Error(t.message);
    return g.name = t.name, g.stack = e.stack.join(`
`) + " ", Object.getOwnPropertyNames(e).forEach((i) => {
      ["message", "name", "stack"].includes(i) || Reflect.defineProperty(g, i, Reflect.getOwnPropertyDescriptor(e, i));
    }), g;
  }
  if (t instanceof Set && A.includes("set")) {
    const e = Array.from(t).reduce((g, i, c) => (g[c] = B(i, A), g), []);
    return /* @__PURE__ */ new Set([...e]);
  }
  if (t instanceof Map && A.includes("map")) {
    const e = B(Object.fromEntries(t), A);
    return new Map(Object.entries(e));
  }
  var o;
  return t;
}
const m = class m {
  constructor(A, o, e) {
    r(this, "_lazyOptions", B(H));
    r(this, "_observerOptions", B(k));
    r(this, "_htmlFieldName");
    r(this, "_imgObserver", null);
    r(this, "_imgId", 1);
    r(this, "_dataImgIdList", []);
    r(this, "_logMsgPrefix", "[vue-lazyload-imgs]:");
    r(this, "_modelHtmlIdx", 0);
    r(this, "_modelHtmlArr", []);
    m.instId++, this._lazyOptions = Object.assign(this._lazyOptions, A), this._observerOptions = Object.assign(this._observerOptions, o), this._htmlFieldName = e;
  }
  setImgSrcToLoadingImg(A, o, e) {
    if (Array.isArray(A) && A.length > 0) {
      let g = "";
      const i = new RegExp("<\\s*img\\s+src\\s*=\\s*.*(\\/)?\\s*>", "gi");
      let c = null;
      A.forEach((E) => {
        if (E.children || typeof E.children == "string") this.setImgSrcToLoadingImg(E.children, o, e);
        else {
          const Q = E.props, s = E.props;
          if (E.type === "img" && Q) g = `inst${m.instId}-${this._imgId++}`, Q[l.IMG_ID] = g, this._dataImgIdList.push(g), Q[l.ORIG_SRC] = Q.src, Q.src = this._lazyOptions.loadingImg;
          else if (s?.innerHTML && (c = s?.innerHTML.match(i)) && (c.forEach((I) => {
            g = `inst${m.instId}-${this._imgId++}`, this._dataImgIdList.push(g);
            const C = `${l.IMG_ID}="${g}"`, d = I.match(/\s*src\s*=\s*['"`]?(.*)['"`].?/i)[1], y = `${l.ORIG_SRC}="${d}"`;
            let n = I.replace(/((\/?)\s*>)/, ` ${C} ${y} $1`);
            n = n.replace(d, this._lazyOptions.loadingImg), s.innerHTML = s.innerHTML.replace(I, n);
          }), o)) {
            if (typeof o == "string") e("update:modelValue", s.innerHTML);
            else if (Array.isArray(o)) {
              if (!this._htmlFieldName) return void console.error(`${this._logMsgPrefix} The Props htmlFieldName cannot be empty! 
Please specify, for example:
<LazyLoadImgs htmlFieldName="content">Some HTML tags</LazyLoadImgs>`);
              this._modelHtmlArr[this._modelHtmlIdx] = { ...o[this._modelHtmlIdx], [this._htmlFieldName]: s.innerHTML }, this._modelHtmlIdx++;
            }
          }
        }
      }), this._modelHtmlArr.length && this._htmlFieldName && e("update:modelValue", this._modelHtmlArr);
    }
    return this._dataImgIdList;
  }
  createImgObserver() {
    return this._imgObserver = new IntersectionObserver((A, o) => {
      A.forEach(async (e) => {
        e.isIntersecting && (this._lazyOptions.delay > 0 ? await this._delayIntersectionCallback(e) : this._intersectionCallback(e));
      });
    }, this._observerOptions), this._imgObserver;
  }
  _intersectionCallback(A) {
    A.isIntersecting && (this._imgObserver?.unobserve(A.target), this._setImgSrc(A.target));
  }
  _delayIntersectionCallback(A) {
    return new Promise((o, e) => {
      if (A.isIntersecting) {
        if (A.target.hasAttribute(l.TIMEOUT_ID)) return;
        let g = setTimeout(() => {
          this._intersectionCallback(A), A.target.removeAttribute(l.TIMEOUT_ID);
        }, this._lazyOptions.delay);
        A.target.setAttribute(l.TIMEOUT_ID, String(g));
      } else this.clearLazyTimeout(A.target);
    });
  }
  _setImgSrc(A) {
    if (A.hasAttribute(l.ORIG_SRC)) {
      let o = A.getAttribute("src"), e = A.getAttribute(l.ORIG_SRC);
      o !== e && (this._lifecycle(a.LOADING, A), A.setAttribute("src", e)), this._listenImgStatus(A, () => {
        A.getAttribute("src") !== this._lazyOptions.errorImg && this._lifecycle(a.LOADED, A);
      }, () => {
        this._imgObserver?.unobserve(A), this._lifecycle(a.ERROR, A), this._lazyOptions.errorImg && A.getAttribute("src") !== this._lazyOptions.errorImg && A.setAttribute("src", this._lazyOptions.errorImg), console.error(`${this._logMsgPrefix} 图片加载失败，图片 url 地址是：${A.getAttribute(l.ORIG_SRC)}`);
      });
    }
  }
  _listenImgStatus(A, o, e) {
    A.onload = o, A.onerror = e;
  }
  clearLazyTimeout(A) {
    A.hasAttribute(l.TIMEOUT_ID) && (clearTimeout(Number(A.getAttribute(l.TIMEOUT_ID))), A.removeAttribute(l.TIMEOUT_ID));
  }
  _lifecycle(A, o) {
    const { lifeFunc: e } = this._lazyOptions;
    switch (A) {
      case a.LOADING:
        o.setAttribute(l.LOAD_STATUS, a.LOADING), e && typeof e[a.LOADING] == "function" && e[a.LOADING]?.(o);
        break;
      case a.LOADED:
        o.setAttribute(l.LOAD_STATUS, a.LOADED), e && typeof e[a.LOADED] == "function" && e[a.LOADED]?.(o);
        break;
      case a.ERROR:
        o.setAttribute(l.LOAD_STATUS, a.ERROR), e && typeof e[a.ERROR] == "function" && e[a.ERROR]?.(o);
    }
  }
};
r(m, "instId", 0);
let u = m;
const Y = p({ name: "LazyLoadImgs", props: { modelValue: { type: [String, Array], required: !1 }, lazyOptions: { type: Object, required: !1, default: () => B(H) }, observerOptions: { type: Object, required: !1, default: () => B(k) }, htmlFieldName: { type: String, required: !1 } }, emits: ["update:modelValue"], setup(t, { emit: A, attrs: o }) {
  const e = Object.assign({}, H, t.lazyOptions), g = Object.assign({}, k, t.observerOptions), i = new u(e, g, t.htmlFieldName);
  let c = u.instId;
  const E = D();
  let Q;
  return f(() => {
    const s = (() => {
      let C = [];
      return T.startsWith("2.6") ? C = E?.slots?.default : C = q().default?.(), C;
    })(), I = t.modelValue;
    i.setImgSrcToLoadingImg(s, I, A);
  }), O(async () => {
    Q = i.createImgObserver();
    const s = document.querySelectorAll(`img[${l.IMG_ID}^=inst${c}-]`);
    await function(I, C) {
      return new Promise((d, y) => {
        const n = new Image();
        n.onload = (M) => d({ img: n, err: null }), n.onerror = (M) => {
          console.warn(`[lazy-load-imgs]: Could not load loadingImg at ${I}.
Now using the default base64 encoding format for preloaded images(loadingImg).`), n.src = R;
          const S = document.querySelectorAll(`img[${l.IMG_ID}^=inst${C}-]`);
          Array.prototype.forEach.call(S, (U) => {
            U.setAttribute("src", R);
          }), y({ img: n, err: M });
        }, n.src = I;
      });
    }(e.loadingImg, c).catch((I) => I), Array.prototype.forEach.call(s, (I) => Q.observe(I));
  }), P(() => {
  }), G(() => {
  }), L(() => {
    const s = document.querySelectorAll(`img[${l.IMG_ID}^=inst${c}-][${l.TIMEOUT_ID}]`);
    Array.prototype.forEach.call(s, (I) => i.clearLazyTimeout(I));
  }), N(() => {
    u.instId--, Q.disconnect();
  }), { instId: c };
} }), v = { class: "lazyload-all-box" }, w = ((t, A) => {
  const o = t.__vccOpts || t;
  for (const [e, g] of A) o[e] = g;
  return o;
})(Y, [["render", function(t, A, o, e, g, i) {
  return j(), x("div", v, [W(t.$slots, "default")]);
}]]), J = [w], Z = (t, A = {}) => {
  Object.assign(H, A), J.forEach((o, e) => {
    t.component(o.name, o);
  });
};
export {
  w as LazyLoadImgs,
  Z as default
};
//# sourceMappingURL=lazy-load-imgs.es.js.map
