var h = Object.defineProperty;
var b = (e, A, o) => A in e ? h(e, A, { enumerable: !0, configurable: !0, writable: !0, value: o }) : e[A] = o;
var E = (e, A, o) => (b(e, typeof A != "symbol" ? A + "" : A, o), o);
import p, { defineComponent as f, getCurrentInstance as D, onBeforeMount as O, onMounted as P, onBeforeUpdate as G, onUpdated as N, onBeforeUnmount as L, onUnmounted as T, version as x, useSlots as j } from "vue";
p.util.warn;
const m = "data:image/jpeg;base64,", R = { loadingImg: m, errorImg: "data:image/jpeg;base64,", delay: 500 }, S = { root: null, rootMargin: "0px", threshold: 0 };
var I = ((e) => (e.LOADING = "loading", e.LOADED = "loaded", e.ERROR = "error", e))(I || {}), n = ((e) => (e.IMG_ID = "data-img-id", e.ORIG_SRC = "data-orig-src", e.TIMEOUT_ID = "data-timeout-id", e.LOAD_STATUS = "data-load-status", e))(n || {});
function d(e, A = ["object", "array", "function", "date", "regexp", "error", "set", "map"]) {
  if (e === null || typeof e != "object" && typeof e != "function")
    return e;
  if (o = e, toString.call(o) === "[object Object]" && A.includes("object"))
    return Object.keys(e).reduce((t, i, g) => (t[i] = d(e[i], A), t), {});
  if (function(t) {
    return toString.call(t) === "[object Array]" || Array.isArray(t);
  }(e) && A.includes("array"))
    return e.reduce((t, i, g) => (t[g] = d(i, A), t), []);
  if (function(t) {
    return toString.call(t) === "[object Function]" || typeof t == "function";
  }(e) && A.includes("function"))
    return new Function("return " + e.toString())();
  if (function(t) {
    return toString.call(t) === "[object Date]" || t instanceof Date;
  }(e) && A.includes("date"))
    return new Date(e.getTime());
  if (function(t) {
    return toString.call(t) === "[object RegExp]" || t instanceof RegExp;
  }(e) && A.includes("regexp"))
    return new RegExp(e.source, e.flags);
  if (function(t) {
    return toString.call(t) === "[object Error]" || t instanceof Error;
  }(e) && A.includes("error")) {
    const t = { ...e };
    Object.getOwnPropertyNames(e).forEach((g) => {
      g === "stack" && (t[g] = e[g].split(`
`)), e.hasOwnProperty("k") && g !== "message" && g !== "name" && g !== "stack" && g !== "constructor" && (t[g] = e[g]);
    });
    const i = new Error(e.message);
    return i.name = e.name, i.stack = t.stack.join(`
`) + " ", Object.getOwnPropertyNames(t).forEach((g) => {
      ["message", "name", "stack"].includes(g) || Reflect.defineProperty(i, g, Reflect.getOwnPropertyDescriptor(t, g));
    }), i;
  }
  if (e instanceof Set && A.includes("set")) {
    const t = Array.from(e).reduce((i, g, c) => (i[c] = d(g, A), i), []);
    return /* @__PURE__ */ new Set([...t]);
  }
  if (e instanceof Map && A.includes("map")) {
    const t = d(Object.fromEntries(e), A);
    return new Map(Object.entries(t));
  }
  var o;
  return e;
}
const y = class {
  constructor(A, o, t) {
    E(this, "_lazyOptions", d(R));
    E(this, "_observerOptions", d(S));
    E(this, "_htmlFieldName");
    E(this, "_imgObserver", null);
    E(this, "_imgId", 1);
    E(this, "_dataImgIdList", []);
    E(this, "_logMsgPrefix", "[vue-lazyload-imgs]:");
    E(this, "_modelHtmlIdx", 0);
    E(this, "_modelHtmlArr", []);
    y.instId++, this._lazyOptions = Object.assign(this._lazyOptions, A), this._observerOptions = Object.assign(this._observerOptions, o), this._htmlFieldName = t;
  }
  setImgSrcToLoadingImg(A, o, t) {
    if (Array.isArray(A) && A.length > 0) {
      let i = "";
      const g = new RegExp("<\\s*img\\s+src\\s*=\\s*.*(\\/)?\\s*>", "gi");
      let c = null;
      A.forEach((Q) => {
        if (Q.children || typeof Q.children == "string")
          this.setImgSrcToLoadingImg(Q.children, o, t);
        else {
          const s = Q?.data?.attrs, a = Q?.data?.domProps;
          Q.tag === "img" && s ? (i = `inst${y.instId}-${this._imgId++}`, s[n.IMG_ID] = i, this._dataImgIdList.push(i), s[n.ORIG_SRC] = s.src, s.src = this._lazyOptions.loadingImg) : a?.innerHTML && (c = a?.innerHTML.match(g)) && (c.forEach((l) => {
            i = `inst${y.instId}-${this._imgId++}`, this._dataImgIdList.push(i);
            const B = `${n.IMG_ID}="${i}"`, r = l.match(/\s*src\s*=\s*['"`]?(.*)['"`].?/i)[1], u = `${n.ORIG_SRC}="${r}"`;
            let C = l.replace(/((\/?)\s*>)/, ` ${B} ${u} $1`);
            C = C.replace(r, this._lazyOptions.loadingImg), a.innerHTML = a.innerHTML.replace(l, C);
          }), o && Array.isArray(o) && this._htmlFieldName && (this._modelHtmlArr[this._modelHtmlIdx] = { ...o[this._modelHtmlIdx], [this._htmlFieldName]: a.innerHTML }, this._modelHtmlIdx++));
        }
      });
    }
    return this._dataImgIdList;
  }
  createImgObserver() {
    return this._imgObserver = new IntersectionObserver((A, o) => {
      A.forEach(async (t) => {
        t.isIntersecting && (this._lazyOptions.delay > 0 ? await this._delayIntersectionCallback(t) : this._intersectionCallback(t));
      });
    }, this._observerOptions), this._imgObserver;
  }
  _intersectionCallback(A) {
    A.isIntersecting && (this._imgObserver?.unobserve(A.target), this._setImgSrc(A.target));
  }
  _delayIntersectionCallback(A) {
    return new Promise((o, t) => {
      if (A.isIntersecting) {
        if (A.target.hasAttribute(n.TIMEOUT_ID))
          return;
        let i = setTimeout(() => {
          this._intersectionCallback(A), A.target.removeAttribute(n.TIMEOUT_ID);
        }, this._lazyOptions.delay);
        A.target.setAttribute(n.TIMEOUT_ID, String(i));
      } else
        this.clearLazyTimeout(A.target);
    });
  }
  _setImgSrc(A) {
    if (A.hasAttribute(n.ORIG_SRC)) {
      let o = A.getAttribute("src"), t = A.getAttribute(n.ORIG_SRC);
      o !== t && (this._lifecycle(I.LOADING, A), A.setAttribute("src", t)), this._listenImgStatus(A, () => {
        A.getAttribute("src") !== this._lazyOptions.errorImg && this._lifecycle(I.LOADED, A);
      }, () => {
        this._imgObserver?.unobserve(A), this._lifecycle(I.ERROR, A), this._lazyOptions.errorImg && A.getAttribute("src") !== this._lazyOptions.errorImg && A.setAttribute("src", this._lazyOptions.errorImg), console.error(`${this._logMsgPrefix} \u56FE\u7247\u52A0\u8F7D\u5931\u8D25\uFF0C\u56FE\u7247 url \u5730\u5740\u662F\uFF1A${A.getAttribute(n.ORIG_SRC)}`);
      });
    }
  }
  _listenImgStatus(A, o, t) {
    A.onload = o, A.onerror = t;
  }
  clearLazyTimeout(A) {
    A.hasAttribute(n.TIMEOUT_ID) && (clearTimeout(Number(A.getAttribute(n.TIMEOUT_ID))), A.removeAttribute(n.TIMEOUT_ID));
  }
  _lifecycle(A, o) {
    const { lifeFunc: t } = this._lazyOptions;
    switch (A) {
      case I.LOADING:
        o.setAttribute(n.LOAD_STATUS, I.LOADING), t && typeof t[I.LOADING] == "function" && t[I.LOADING]?.(o);
        break;
      case I.LOADED:
        o.setAttribute(n.LOAD_STATUS, I.LOADED), t && typeof t[I.LOADED] == "function" && t[I.LOADED]?.(o);
        break;
      case I.ERROR:
        o.setAttribute(n.LOAD_STATUS, I.ERROR), t && typeof t[I.ERROR] == "function" && t[I.ERROR]?.(o);
    }
  }
};
let H = y;
E(H, "instId", 0);
function q(e, A, o, t, i, g, c, Q) {
  var s, a = typeof e == "function" ? e.options : e;
  if (A && (a.render = A, a.staticRenderFns = o, a._compiled = !0), t && (a.functional = !0), g && (a._scopeId = "data-v-" + g), c ? (s = function(r) {
    (r = r || this.$vnode && this.$vnode.ssrContext || this.parent && this.parent.$vnode && this.parent.$vnode.ssrContext) || typeof __VUE_SSR_CONTEXT__ > "u" || (r = __VUE_SSR_CONTEXT__), i && i.call(this, r), r && r._registeredComponents && r._registeredComponents.add(c);
  }, a._ssrRegister = s) : i && (s = Q ? function() {
    i.call(this, (a.functional ? this.parent : this).$root.$options.shadowRoot);
  } : i), s)
    if (a.functional) {
      a._injectStyles = s;
      var l = a.render;
      a.render = function(r, u) {
        return s.call(u), l(r, u);
      };
    } else {
      var B = a.beforeCreate;
      a.beforeCreate = B ? [].concat(B, s) : [s];
    }
  return { exports: e, options: a };
}
const v = q(f({ name: "LazyLoadImgs", props: { value: { type: [String, Array], required: !1 }, lazyOptions: { type: Object, required: !1, default: () => d(R) }, observerOptions: { type: Object, required: !1, default: () => d(S) }, htmlFieldName: { type: String, required: !1 } }, emits: [], setup(e, { emit: A, attrs: o }) {
  const t = Object.assign({}, R, e.lazyOptions), i = Object.assign({}, S, e.observerOptions), g = new H(t, i, e.htmlFieldName);
  let c = H.instId;
  const Q = D();
  let s;
  return O(() => {
    const a = (() => {
      let B = [];
      return x.startsWith("2.6") ? B = Q?.slots?.default : B = j().default?.(), B;
    })(), l = e.value;
    g.setImgSrcToLoadingImg(a, l, A);
  }), P(async () => {
    s = g.createImgObserver();
    const a = document.querySelectorAll(`img[${n.IMG_ID}^=inst${c}-]`);
    await function(l, B) {
      return new Promise((r, u) => {
        const C = new Image();
        C.onload = (k) => r({ img: C, err: null }), C.onerror = (k) => {
          console.warn(`[lazy-load-imgs]: Could not load loadingImg at ${l}.
Now using the default base64 encoding format for preloaded images(loadingImg).`), C.src = m;
          const M = document.querySelectorAll(`img[${n.IMG_ID}^=inst${B}-]`);
          Array.prototype.forEach.call(M, (U) => {
            U.setAttribute("src", m);
          }), u({ img: C, err: k });
        }, C.src = l;
      });
    }(t.loadingImg, c).catch((l) => l), Array.prototype.forEach.call(a, (l) => s.observe(l));
  }), G(() => {
  }), N(() => {
  }), L(() => {
    const a = document.querySelectorAll(`img[${n.IMG_ID}^=inst${c}-][${n.TIMEOUT_ID}]`);
    Array.prototype.forEach.call(a, (l) => g.clearLazyTimeout(l));
  }), T(() => {
    H.instId--, s.disconnect();
  }), { instId: c };
} }), function() {
  var e = this, A = e._self._c;
  return e._self._setupProxy, A("div", { staticClass: "lazyload-all-box" }, [e._t("default")], 2);
}, [], !1, null, "7d66dde6", null, null).exports, W = [v], J = (e, A = {}) => {
  Object.assign(R, A), W.forEach((o, t) => {
    e.component(o.name, o);
  });
};
export {
  v as LazyLoadImgs,
  J as default
};
//# sourceMappingURL=lazy-load-imgs.es.js.map
